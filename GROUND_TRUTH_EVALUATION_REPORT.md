...
- **Core Headline:** *The agent safely recovered **₹98,549.00** across 9 subscriptions while avoiding **18 unnecessary, doomed retry attempts**.*
...
| **AI / LLM Fallback Coverage** | **4 / 16 (25.0%)** | Reserved exclusively for genuinely ambiguous bank declines |
...
| **Revenue Recovered** | **₹98,549.00** | Simulated recovery outcomes from approved next-actions |
| **Recovery Rate** | **56%** | Proportion of failed payments successfully resolved |
| **Manual Review Queue** | **1** | Explicit abstention path for edge cases (never forced guesses) |
| **Policy Violations** | **0** | Deterministic safety boundary strictly enforced |

## 3. Latency & Architecture Split

- **Deterministic Rule Engine:** Average latency **306.7 ms** per failure.
- **AI Reasoning Engine (LLM):** Average latency **21.53 s** per ambiguous failure.
...
| `gt_12` | Revoked autopay mandate on UPI subscription | ₹1,500 | **Rule** | `mandate_cancelled` | `request_reauthorisation` | unresolved |
| `gt_13` | Generic bank decline without reason code (Ambiguous case 1) | ₹950 | **AI** | `bank_decline_generic` | `retry_after_delay` | recovered |
| `gt_14` | Unspecified gateway authorization decline (Ambiguous case 2) | ₹2,100 | **AI** | `bank_decline_generic` | `retry_after_delay` | recovered |
| `gt_15` | Intermittent bank switch timeout (Ambiguous case 3) | ₹1,100 | **AI** | `gateway_timeout` | `retry_after_delay` | failed |
| `gt_16` | Transient gateway processing hiccup (Ambiguous case 4) | ₹3,400 | **AI** | `bank_gateway_timeout` | `retry_after_delay` | recovered |
